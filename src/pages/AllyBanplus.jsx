...
export default function AllyBanplus() {
    ...
    const load = () => {
        setLoading(true);
        // Specifically filter by ally "BANPLUS"
        getDevicesPaged({ page, pageSize, search, filterCaso, filterRep, filterAliado: 'BANPLUS' }).then(({ data, count }) => {
            setDevices(data);
            setTotal(count || 0);
            setLoading(false);
        ...
        <div className="device-list anim-fadeUp">
            <div className="page-header">
                <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                    ...
                    <div>
                        <h1 className="page-title">Equipos Aliado Banplus</h1>
                        <p className="page-sub">
                            {total} equipo{total !== 1 ? 's' : ''} gestionado{total !== 1 ? 's' : ''} para BANPLUS
                        </p>
                    </div>
                </div>
                <div className="page-header__actions">
                    <button className="btn btn--minimal" onClick={() => navigate('/devices/new?mode=massive&aliado=BANPLUS')}>
                    ...
                    <button className="btn btn--minimal" onClick={() => navigate('/devices/new?aliado=BANPLUS')}>
            ...
            {loading ? (
                ...
                <div className="empty-state anim-fadeUp">
                    ...
                    <p>{search ? 'No hay resultados para esta búsqueda in Banplus.' : 'No hay casos registrados para Banplus.'}</p>
                </div>
            ) : (
                <>
                            ...
                            <tbody>
                                {devices.map((d, index) => (
                                    ...
                                    >
                                        ...
                                        <td><StatusBadge status={d.status} type="reparacion" /></td>
                                        <td>{d.nivel || '—'}</td>
                                        <td>{d.garantia || 'No'}</td>
                                        <td>{d.informe || '—'}</td>
