...
export default function AllyPlatco() {
    ...
    const load = () => {
        setLoading(true);
        // Specifically filter by ally "PLATCO"
        getDevicesPaged({ page, pageSize, search, filterCaso, filterRep, filterAliado: 'PLATCO' }).then(({ data, count }) => {
            setDevices(data);
            setTotal(count || 0);
            setLoading(false);
    ...
    return (
        <div className="device-list anim-fadeUp">
            <div className="page-header">
                <div>
                    <h1 className="page-title">Lista de Equipos: PLATCO</h1>
                    <p className="page-sub">
                        {total} equipo{total !== 1 ? 's' : ''} gestionado{total !== 1 ? 's' : ''} para PLATCO
                    </p>
                </div>
            </div>
