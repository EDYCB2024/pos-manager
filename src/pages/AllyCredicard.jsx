...
export default function AllyCredicard() {
    ...
    const load = () => {
        setLoading(true);
        // Specifically filter by ally "CREDICARD"
        getDevicesPaged({ page, pageSize, search, filterCaso, filterRep, filterAliado: 'CREDICARD' }).then(({ data, count }) => {
            setDevices(data);
            setTotal(count || 0);
            setLoading(false);
            ...
            <div className="page-header">
                <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
                    <div style={{
                        ...
                        <img src={credicardLogo} alt="Credicard" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
                    </div>
                    <div>
                        <h1 className="page-title">Equipos Aliado Credicard</h1>
                        <p className="page-sub">
                            {total} equipo{total !== 1 ? 's' : ''} gestionado{total !== 1 ? 's' : ''} para CREDICARD
                        </p>
                    </div>
                </div>
